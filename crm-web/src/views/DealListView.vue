<template>
  <section>
    <div class="toolbar">
      ...
      <button class="mobile-search-toggle" type="button" @click="mobileFilterOpen = !mobileFilterOpen">
        搜索
      </button>
    </div>

    <FilterPanel v-model:expanded="filterExpanded" :mobile-open="mobileFilterOpen">
      <el-input v-model="filters.customerCode" clearable placeholder="客户编号" @keyup.enter="searchRows" />
      <el-input v-model="filters.customerName" clearable placeholder="客户姓名" @keyup.enter="searchRows" />
      <el-select v-model="salesEmployeeCode" filterable placeholder="所有销售">
        <el-option label="所有销售" value="" />
        <el-option
          v-for="sales in salesUsers"
          :key="sales.employeeCode"
          :label="`${sales.name}（${sales.employeeCode}）`"
          :value="sales.employeeCode"
        />
      </el-select>
      <el-date-picker
        v-model="dateRange"
      ...
      />
      <template #advanced>
        <el-input v-model="filters.dealCode" clearable placeholder="成交编号" @keyup.enter="searchRows" />
        <el-select v-model="filters.status" clearable placeholder="成交状态">
          <el-option label="已交定金" value="DEPOSIT_PAID" />
          <el-option label="退单" value="REFUNDED" />
          <el-option label="已落地" value="LANDED" />
        </el-select>
        <el-input v-model="filters.keyword" clearable placeholder="行程/报价/退单备注" @keyup.enter="searchRows" />
      </template>
      <template #actions>
    ...
    <div class="panel desktop-table">
      <el-table :data="rows" width="100%" v-loading="loading">
        <el-table-column prop="dealCode" label="成交编号" min-width="120" />
    ...
    <div class="mobile-list" v-loading="loading">
      <article v-for="row in rows" :key="row.dealCode" class="deal-card">
        <div class="card-head">
          <div>
      ...
      <el-empty v-if="!loading && rows.length === 0" description="暂无成交记录" />
    ...
    <el-dialog v-model="detailVisible" title="成交详情" width="min(720px, 94vw)">
      <div v-if="currentDeal" class="deal-detail">
        <div class="detail-hero">
          <div>
            <strong>{{ currentDeal.customerName }}</strong>
            <span>{{ currentDeal.customerCode }} · {{ currentDeal.dealCode }}</span>
          </div>
          <StatusTag :status="currentDeal.status" />
        </div>
    ...
    <el-dialog v-model="cancelVisible" title="退单信息" width="min(520px, 92vw)">
      <el-form label-position="top">
        <el-form-item label="退单金额">
          <el-input v-model="cancelForm.refundAmount" placeholder="例如：500" />
        </el-form-item>
        <el-form-item label="退款时间">
          <el-date-picker v-model="cancelForm.refundedAt" class="full-field" type="datetime" value-format="YYYY-MM-DD HH:mm" placeholder="默认当前时间" />
        </el-form-item>
        <el-form-item label="退单备注">
          <el-input v-model="cancelForm.remark" type="textarea" :rows="3" placeholder="例如：客户取消行程，定金已退" />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-button @click="cancelVisible = false">取消</el-button>
        <el-button type="danger" :loading="cancelSaving" @click="submitCancel">确认退单</el-button>
      </template>
    </el-dialog>
...
async function fetchRows() {
  loading.value = true
  try {
    const res = await listDeals({ ...queryParams(), page: page.current, pageSize: page.size })
    const data = res.data || {}
...
async function cancelRow(row) {
  try {
    await ElMessageBox.confirm(`确认将成交记录 ${row.dealCode} 标记为退单吗？`, '退单确认', {
...
async function submitCancel() {
  ...
  try {
    await cancelDeal(cancelTarget.value.dealCode, {
      remark: cancelForm.remark || '退单',
      refundAmount: cancelForm.refundAmount,
      refundedAt: cancelForm.refundedAt
    })
...
function queryParams() {
  return {
    ...
    salesEmployeeCode: salesEmployeeCode.value,
    startDate: dateRange.value?.[0],
    endDate: dateRange.value?.[1]
  }
}

function isAutoDeal(row) {
  return row?.dealCode?.startsWith('AUTO-')
}
...
onMounted(async () => {
  await Promise.all([fetchRows(), fetchSalesUsers()])
})
</script>
